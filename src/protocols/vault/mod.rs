...
impl Protocol for VaultProtocol {
    ...
    async fn authenticate(&self, target: &Target, credential: &Credential, timeout_dur: Duration, proxy: &Option<ProxyConfig>) -> AuthResult {
        ...
        let url = format!("{}://{}:{}{}", scheme, target.host, target.port, "/v1/secret/access");
        http_basic_auth(target, credential, timeout_dur, proxy, &url, "vault", start).await
    }
}
