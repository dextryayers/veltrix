...
impl Protocol for ConsulProtocol {
    fn name(&self) -> &'static str { "consul" }
    ...
    async fn authenticate(&self, target: &Target, credential: &Credential, timeout_dur: Duration, proxy: &Option<ProxyConfig>) -> AuthResult {
        ...
        let url = format!("{}://{}:{}{}", scheme, target.host, target.port, "/v1/kv/?keys");
        http_basic_auth(target, credential, timeout_dur, proxy, &url, "consul", start).await
    }
}
