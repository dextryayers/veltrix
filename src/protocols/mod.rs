...
pub fn get_protocol(name: &str) -> Option<Box<dyn Protocol>> {
    match name.to_lowercase().as_str() {
        ...
        "http" | "http-basic" | "http-form" => Some(Box::new(http::HttpProtocol)),
        _ => None,
    }
}
...
mod tests {
    ...
    fn test_list_protocols() {
        let protocols = vec!["ssh", "ftp", "telnet", "smtp", "pop3", "rdp", "mysql", "http"];
...
pub fn list_protocols() -> Vec<&'static str> {
    vec!["ssh", "ftp", "telnet", "smtp", "pop3", "rdp", "mysql", "http"]
}
